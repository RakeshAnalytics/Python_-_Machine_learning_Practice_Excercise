...
CREATE TABLE Customers (
    ...
    email VARCHAR(55),  -- fixed typo: "emaril" → "email"
    phone_number VARCHAR(15),  -- fixed casing
...
-- 4. Create Screenings Table (after Film & Rooms)
...
-- 5. Create Seats Table (after Rooms)
CREATE TABLE Seats (
    seat_id INT PRIMARY KEY,
    room_id INT,
    seat_number VARCHAR(55),
    `row_number` VARCHAR(28),
    FOREIGN KEY (room_id) REFERENCES Rooms(room_id)
);

-- 6. Create Bookings Table (after Customers & Screenings)
...
-- 7. Create Reserved_Seats Table (after Bookings, Seats, and Screenings)
...
-- -1. Create Film Table
CREATE TABLE Films (
	name VARCHAR(120), length_min INT
    );
...
CREATE TABLE Customers (
	first_name VARCHAR(55), last_name VARCHAR(55), email VARCHAR(32)
    );
...
CREATE TABLE screenings  (
	film_id INT, room_id INT, start_time DATETIME
    );
...
CREATE TABLE seats (
	seat_row VARCHAR(10), number INT, room_id INT
    );
...
CREATE TABLE bookings (
	screening_id INT , customer_id INT
    );
...
-- 7. CREATE TABLE RESERVED SEAT

CREATE TABLE reserved_seat (
	booking_id INT , seat_id INT
    );
